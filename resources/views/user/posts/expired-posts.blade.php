                    ...
                    <div class="p-6 bg-white border-b border-gray-200  flex justify-between">
                        <div class="flex">
                            <div class="article-thumbnail w-16 mr-6">
                                <img class=""
                                    src="{{ asset('storage/posts/' . $expiredPost->thumbnail) }}" />
                            </div>
                            <div>
                        ...
                        <div>
                            <form id="delete_{{ $expiredPost->id }}"
                                action="{{ route('user.expired-posts.destroy', ['post' => $expiredPost->id]) }}"
                                method="post">
                                @csrf
                        ...
                        <div>
                            <form action="{{ route('user.expired-posts.restore', ['post' => $expiredPost->id]) }}"
                                method="post">
                                @csrf
                                @method('patch')
                                <button
                                    class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">復元する</button>
                            </form>
                        </div>
                    </div>
                @endforeach
